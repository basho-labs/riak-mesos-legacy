...
%% This is where our customizations lie
start_protos(Name, [Proto | Ps], Node, Ls) ->
    Mod = list_to_atom(Proto ++ "_dist"),
	%% Hackish as hell
	%% It's to detect if it's using our erl_epmd, or a stock erl_epmd
	%% -Sargun Dhillon, 8/4/2015
	Fun =
		case catch erl_epmd:mesos_mode() of
			true ->
				  erlang:display("Has Mesos Mode"),
				  fun () -> Mod:listen(Node) end;
			_ ->
				erlang:display("Does not Mesos Mode"),
				fun () -> Mod:listen(Name) end
		end,
	%%
    case catch Fun() of
