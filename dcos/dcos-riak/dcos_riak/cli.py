...
def usage():
    ...
    print("    proxy config [--zk <host:port> [--chroot [--disable-super-chroot]]]")
    print("    proxy install [--zk <host:port> [--chroot [--disable-super-chroot]]]")
...
def install_director(framework, cluster, zookeeper, chroot_flag, disable_super_chroot_flag):
    try:
        director_conf = generate_director_config(framework, cluster, zookeeper, chroot_flag, disable_super_chroot_flag)
...
def generate_director_config(framework, cluster, zookeeper, chroot_flag, disable_super_chroot_flag):
    try:
        ...
        return '{"id": "/' + framework + '-director","cmd": "./riak_mesos_director/director_linux_amd64","cpus": 0.5,"mem": 1024.0,"ports": [0, 0, 0, 0],"instances": 1,"constraints": [["hostname", "UNIQUE"]],"acceptedResourceRoles": ["slave_public"],"env": {"USE_CHROOT": "' + str(chroot_flag).lower() + '","USE_SUPER_CHROOT": "'+ str(not disable_super_chroot_flag).lower() + '","FRAMEWORK_HOST": "' + framework_host + '","FRAMEWORK_PORT": "' + explorer_port + '","DIRECTOR_ZK": "' + zookeeper + '","DIRECTOR_FRAMEWORK": "' + framework + '","DIRECTOR_CLUSTER": "' + cluster + '"},"uris": ["http://riak-tools.s3.amazonaws.com/riak-mesos/coreos/riak_mesos_director_linux_amd64_0.1.1.tar.gz"],"healthChecks": [{"protocol": "HTTP","path": "/health","gracePeriodSeconds": 3,"intervalSeconds": 10,"portIndex": 2,"timeoutSeconds": 10,"maxConsecutiveFailures": 3}]}'
    except errors.DCOSException as e:
        print(e.message)
        raise CliError("Unable to create marathon app")
    except:
...
def run(args):
    ...
    args, chroot_flag = extract_flag(args, "--chroot")
    ...
    if cmd == "cluster list" or cmd == "cluster":
        ...
        if help_flag:
            ...
            print(generate_director_config(framework, cluster, zk, chroot_flag, disable_super_chroot_flag))
        ...
        if help_flag:
            ...
            install_director(framework, cluster, zk, chroot_flag, disable_super_chroot_flag)
